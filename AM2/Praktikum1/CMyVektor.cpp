...
CMyVektor gradient(CMyVektor x, double (*funktion)(CMyVektor x)){
    ...
    for (unsigned i = 0; i < x.get_dimension(); i++) {
        ...
        // das h von der i-ten Stelle wieder Subtrahieren um die Ableitungen nach den
        // anderen Variablen mit der Ausgangsfunktion zu Approximieren
        x.set_component(i, x.get_component(i) - h);
    }
    return g;
}
...
CMyVektor gradientVerfahren(CMyVektor x, double (*funktion) (CMyVektor x), double lambda){
    ...
    while(true) {
        if(schritt >= 25) { // 1. ABBRUCHBEDINGUNG
            ...
            if(f_next > f) { // Falls f(x) groesser als f(x_neu) test mit doppelter Schrittweite
